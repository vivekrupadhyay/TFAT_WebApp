...
/**
 * fdfasdfsdafasfs
 * @license jqGrid  4.4.1  - jQuery Grid by ccc 
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		$("<div class='loading ui-state-default"+postfix_css_subgrid+" ui-state-active' id='load_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(gv);
		...
		addRowNum = function (pos,irow,pG,rN) {
			...
			return "<td role=\"gridcell\" class=\"ui-state-default"+postfix_css_subgrid+" jqgrid-rownum\" "+prp+">"+v+"</td>";
		},
		...
		if(this.p.id && this.p.id.indexOf("_t") >= 0) {
		    postfix_css_subgrid = '-pp-isms';
		} else {
		    postfix_css_subgrid = '';
		}
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default"+postfix_css_subgrid+" ui-th-column ui-th-"+dir+"'"+ tooltip+">";
		...
		$(grid.hDiv)
			...
			.addClass("ui-state-default"+postfix_css_subgrid+" ui-jqgrid-hdiv")
			.append(hb);
		$(hb).append(hTable);
		hTable = null;
		...
		if(ts.p.pager){
			...
			$(ts.p.pager).css({width: grid.width+"px"}).appendTo(eg).addClass('ui-state-default'+postfix_css_subgrid+' ui-jqgrid-pager ui-corner-bottom');
		...
		.addClass("ui-jqgrid-titlebar ui-widget-header"+postfix_css_subgrid+" ui-corner-top"+postfix_css_subgrid+" ui-helper-clearfix"+postfix_css_subgrid);
		...
		if( ts.p.toolbar[0] ) {
			...
			if(ts.p.toolbar[1]=="both") {
				...
				$(grid.uDiv).insertBefore(grid.hDiv).addClass("ui-userdata ui-state-default"+postfix_css_subgrid).attr("id","t_"+this.id);
				$(grid.ubDiv).insertAfter(grid.hDiv).addClass("ui-userdata ui-state-default"+postfix_css_subgrid).attr("id","tb_"+this.id);
				if(hg)  {$(grid.ubDiv).hide();}
			} else {
				$(grid.uDiv).width(grid.width).addClass("ui-userdata ui-state-default"+postfix_css_subgrid).attr("id","t_"+this.id);
			}
		...
		if(ts.p.toppager) {
			...
			$(grid.topDiv).insertBefore(grid.hDiv).addClass('ui-state-default'+postfix_css_subgrid+' ui-jqgrid-toppager').width(grid.width);
		...
		if(ts.p.footerrow) {
			...
			if(ts.p.rownumbers) { grid.footers[0].className = 'ui-state-default'+postfix_css_subgrid+' jqgrid-rownum'; }
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(ni){
						...
						row[row.length] = "<td role=\"gridcell\" class=\"ui-state-default"+postfix_css_subgrid+" jqgrid-rownum\" "+prp+">0</td>";
					}
					if(gi) {
...
$.jgrid.extend({
	...
	GridUnload : function(){
		return this.each(function(){
			...
			if (this.p.pager) {
				$(this.p.pager).empty().removeClass("ui-state-default"+postfix_css_subgrid+" ui-jqgrid-pager corner-bottom");
			}
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(){
				...
				th = $("<th role='columnheader' class='ui-state-default"+postfix_css_subgrid+" ui-th-column ui-th-"+$t.p.direction+"'></th>");
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$colHeader = $('<th>').attr({role: "columnheader"})
						.addClass("ui-state-default"+postfix_css_subgrid+" ui-th-column-header ui-th-"+ts.p.direction)
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height:'+hth+'px;" class="frozen-div ui-state-default'+postfix_css_subgrid+' ui-jqgrid-hdiv"></div>');
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		if(mopt.buttons.length > 0) {
			for(var i=0;i<mopt.buttons.length;i++) {
				...
				buttstr += "<a href='javascript:void(0)' id='"+mopt.buttons[i].id+"' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all'>"+mopt.buttons[i].text+"</a>";
			}
		}
		...
		cnt += c_b ? "<div class='ui-widget-content ui-helper-clearfix' style='text-align:"+mopt.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a href='javascript:void(0)' id='closedialog' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all'>"+c_b+"</a>"+buttstr+"</div>" :
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				bS  ="<a href='javascript:void(0)' id='"+fid+"_search' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all fm-button-icon-right ui-reset'><span class='ui-icon ui-icon-search'></span>"+p.Find+"</a>",
				bC  ="<a href='javascript:void(0)' id='"+fid+"_reset' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all fm-button-icon-left ui-search'><span class='ui-icon ui-icon-arrowreturnthick-1-w'></span>"+p.Reset+"</a>",
				bQ = "", tmpl="", colnm, found = false, bt, cmi=-1;
				if(p.showQuery) {
					bQ ="<a href='javascript:void(0)' id='"+fid+"_query' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all fm-button-icon-left'><span class='ui-icon ui-icon-comment'></span>Query</a>";
				}
				if(!p.columns.length) {
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				// buttons at footer
				var bP = "<a href='javascript:void(0)' id='"+bp+"' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>",
				bN = "<a href='javascript:void(0)' id='"+bn+"' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>",
				bS  ="<a href='javascript:void(0)' id='sData' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all'>"+p.bSubmit+"</a>",
				bC  ="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all'>"+p.bCancel+"</a>";
				...
				if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
					bS  ="<a href='javascript:void(0)' id='sNew' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all' style='z-index:1002'>"+p.bYes+"</a>";
					bN  ="<a href='javascript:void(0)' id='nNew' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all' style='z-index:1002'>"+p.bNo+"</a>";
					bC  ="<a href='javascript:void(0)' id='cNew' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all' style='z-index:1002'>"+p.bExit+"</a>";
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				// buttons at footer
				bP = "<a href='javascript:void(0)' id='"+bp+"' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>",
				bN = "<a href='javascript:void(0)' id='"+bn+"' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>",
				bC  ="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all'>"+p.bClose+"</a>";
				if(maxRows >  0) {
					var sd=[];
					$.each($(tbl)[0].rows,function(i,r){
						sd[i] = r;
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				// buttons at footer
				tbl += "</tbody></table></div>";
				var bS  = "<a href='javascript:void(0)' id='dData' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all'>"+p.bSubmit+"</a>",
				bC  = "<a href='javascript:void(0)' id='eData' class='fm-button ui-state-default"+postfix_css_subgrid+" ui-corner-all'>"+p.bCancel+"</a>";
	...
	return this.each(function(){
		...
		var subGridXml = function(sjxml, sbid){
			...
			for (i = 0; i<ts.p.subGridModel[0].name.length; i++) {
				tddiv = $("<th class='ui-state-default"+postfix_css_subgrid+" ui-th-subgrid ui-th-column ui-th-"+ts.p.direction+"'></th>");
